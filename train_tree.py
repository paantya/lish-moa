...
from src.tree.xgb import get_xgboost1
...
def run(cfg: DictConfig) -> None:
    ...
    for seed in tqdm(SEED, leave=verbose):
        ...
        print(f"train.head(): {train.head(1)}")
        train = preprocess_data(train[[i for i in train.columns if i not in train_targets_scored.columns or i == 'sig_id']], cfg.model.patch1)
        test = preprocess_data(test, cfg.model.patch1)
        ...
        return_run_k_fold = get_xgboost1(train=train, targets=target, test=test, sub=sample_submission, xgb_params=xgb_params, NFOLDS=2, verbosity=2)
